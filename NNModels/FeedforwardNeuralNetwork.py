...
class Layer(object):
    ...
    def get_parameters(self):
        return self.typ, self.node_num, self.prev_layer
...
class NeuralNetwork(object):
    def __init__(self, input_data, target_data):
        ...
        self.output_layer = OutputLayer(typ='sigm', target_vector=self.target_vector, prev_layer=self.input_layer)

    def fnnpropagate(self):
        for n, layer in enumerate(self.hidden_layer_list):
    ...
    def set_hidden_layer(self, typ, node_num):
        prev_layer = self.hidden_layer_list[-1] if bool(self.hidden_layer_list) else self.input_layer
        self.hidden_layer_list.append(Layer(typ, node_num, prev_layer))
        self.output_layer.prev_layer = self.hidden_layer_list[-1]
        pass
...
NN.set_hidden_layer('relu', 5)
NN.set_hidden_layer('sigm', 10)
NN.set_hidden_layer('relu', 7)
NN.fnnpropagate()
print(NN.hidden_layer_list[0].W, NN.hidden_layer_list[1].W)
